...
Network = () ->
  ...
  setupData = () ->
    ...
    # update nodes with nmap data
    for i in [0...nodes.length]
      n = nodes[i]
      
      if n.sku of nmap_raport["hosts_up"]
         nodes[i]["ip"] = nmap_raport["hosts_up"][n.sku].ip
         nodes[i]["online"] = true
         nodes[i]["nmap"] = nmap_raport["hosts_up"][n.sku]
    ...
    circleRadius = d3.scale.sqrt().range([11, 18]).domain(countExtent)
...
# Public function to mark nodes
  ...
  showDetails = (d,i) ->
    ...
    #if d.ip
    content += '<p class="main">IP: ' + d.ip + '</span></p>'
