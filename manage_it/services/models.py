...
SERVICE_TYPES = (
    (1, "Communications"),
    (2, "Security"),
    (3, "Servers, Data, Backup"),
    (4, "Software & Business Applications"),
    (5, "Web & Collaboration"),
...
class Service(models.Model, AuditMixin):
    ...
    user_groups = models.ManyToManyField(Group, null=True, blank=True)
    ...
    def is_used_by(self, user):
        ...
        user_groups_id = self.user_groups.values_list("id")[0]
...
class SLA(models.Model, AuditMixin):
    ...
    service_classes = models.ManyToManyField(DataForm, 
    ...
    customer_requirements = models.TextField(_(u"Customer Requirements"),
          null=True, blank=True)
    provider_requirements = models.TextField(_(u"Provider Requirements"),
          null=True, blank=True)
    
    coverege_time = models.TextField(_(u"Coverege Time"),
          null=True, blank=True)
    
    support_time = models.TextField(_(u"Support Time"),
          null=True, blank=True)
    
    maintenance = models.TextField(_(u"Maitenance Management"),
          null=True, blank=True)
        
    reviewing = models.TextField(_(u"Reporting, Reviewing and Auditing"),
          null=True, blank=True)
    review_period = models.SmallIntegerField(_(u"review period"), 
           choices=((1,1), (2,2)),
           default=1,
           help_text="in years",
           )
    ...
    terminated = models.DateField(_(u"Service terminated"),
           null=True, blank=True)
    ...
    def __unicode__(self):
        return u"%s privided by %s - %s" % (self.service, self.provider, self.active)

    @models.permalink
    def get_absolute_url(self):
        return ('sla_view', [str(self.service.owner.url), str(self.service_id), str(self.pk)])
...
class SLA_billing(models.Model):
    ...
    billing_amount = models.SmallIntegerField(_(u"amount"), )
    billing_currency = models.SmallIntegerField(_(u"billing currency"),
           default=1,
           choices=CURRENCIES)
    billing_method = models.SmallIntegerField(_(u"billing method"), choices=PAYMENT_METHODS)
    billing_period = models.SmallIntegerField(_(u"billing period"), 
           choices=((1,"1 month"), (2,"2 months"), (6,"6 months"), (12,"1 year"), (24,"2 years")),
           default=12,
           null=True, blank=True,)
    account = models.TextField(_(u"account/card nr"), null=True, blank=True)
...
class Document(models.Model):
    ...
    document_type = models.SmallIntegerField(_(u"resource type"), choices=DOCUMENT_TYPES)
    name = models.CharField(_(u"name"), max_length=32)
    
    sensitivity = models.SmallIntegerField(_(u"sensitivity"), choices=DOCUMENT_SENSITIVITY)
    
    created_at = models.DateTimeField(default=datetime.datetime.now())
    created_by = models.ForeignKey(User)
    
    doc = models.FileField("file", 
