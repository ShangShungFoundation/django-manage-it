...
class State(models.Model):
    ...
    def __unicode__(self):
        return "%s - %s" % (self.name, self.exclusive and _(u'exl') or _(u'inc'))
...
class Item(models.Model):
    ...
    item_class = models.ForeignKey(DataForm, 
        verbose_name=_(u"item class"),
        related_name="related_forms")
    
    property_number = models.CharField(_(u"asset number (SKU)"), max_length=48, unique=True)
    serial_number = models.CharField(_(u"serial number"), max_length=48, null=True, blank=True)
    
    location = models.ForeignKey(Location, verbose_name=_(u"location"), null=True, blank=True)
    position = models.TextField(_(u"position"), null=True, blank=True) 
    
    function = models.TextField(_(u"function"), null=True, blank=True)  
    ...
    def __unicode__(self):                
        return "%s %s - %s" % (self.property_number, self.item_template, self.location)
...
class Person(models.Model):
    ...
    second_last_name = models.CharField(_(u"second last name"), max_length=32, blank=True, null=True)
    first_name = models.CharField(_(u"first name"), max_length=32)
    second_name = models.CharField(_(u"second name or initial"), max_length=32, blank=True, null=True)
...
class Owner(models.Model):
    ...
    email = models.EmailField(_(u"second last name"), max_length=32, blank=True, null=True)
...
class Resource(models.Model):
    item = models.ForeignKey(Item, verbose_name=_(u"item"))
    resurce_type = models.SmallIntegerField(_(u"resource type"), choices=RESOURCE_TYPES)
...
class AssetRequest(models.Model):
    replaced_item = models.ForeignKey(Item, 
    ...
    status = models.SmallIntegerField(_(u"status"), 
         choices=ASSET_REQUEST_STATUSES)
    
    description = models.TextField(_(u"description"), null=True, blank=True)  

    def __unicode__(self):
        return u"new %s requested by %s at %s" % (self.item_class,  self.created_by, self.created_at)