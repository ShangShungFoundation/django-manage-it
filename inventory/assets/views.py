...
def categories(request):
    ...
    categories = list(DataForm.objects.all())
...
def get_json(request, slug):
    ...
    it = Item.objects.select_related("item_template", "item_class", "users", "owner", "state").get(property_number=slug)
...
def edit(request, slug):
    ...
    detail_form = create_form(request, form=instance.item_class, submission=submission)
    resource_form = ResourcesSet(request.POST or None, request.FILES or None)
...
@staff_member_required
def asset_request(request, object_id):
