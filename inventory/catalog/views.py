...
def export_xsl(request):
    ...
    # making headers
    column_names = ["SKU", "serial number", "location", "owner", "active", "state", "function", "description", "brand", "model", "notes",]
    
    if results["query"] and items:
        # import ipdb; ipdb.set_trace()
        # get properties from the last item
        extra_column_names = get_answers(items.reverse()[0].property_number).keys()
    ...
    query_to_remove =  [u"item_set-TOTAL_FORMS", u"item_set-MAX_NUM_FORMS", u"item_set-INITIAL_FORMS" ]
...
def list_json(request):
    ...
    items = Item.objects.select_related("item_template", "item_class", "users", "owner", "state").all()
